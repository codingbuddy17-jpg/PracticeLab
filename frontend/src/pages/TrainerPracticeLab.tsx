...
import { ChevronLeft, Plus, Upload, Download, FileCheck, BarChart2, Key, Loader } from 'lucide-react'
...
type View = 'home' | 'answer-keys' | 'create-batch' | 'batch-detail' | 'drg-review' | 'results' | 'analytics'
...
function CreateBatchView({ onCreated }: { onCreated: (id: number) => void }) {
  ...
  async function handleCreate() {
    ...
    const coders = form.coders.split('\n').map(s => s.trim()).filter(Boolean)
    ...
    const perCoder = Math.min(form.charts_per_coder, pool.with_answer_key)
    setCreating(true)
    try {
  ...
  const coderList = form.coders.split('\n').map(s => s.trim()).filter(Boolean)
  const needed = coderList.length * form.charts_per_coder
  ...
  return (
    <div style={styles.section}>
      ...
      {/* Live pool count */}
      {pool && (
        <div style={needed > available ? styles.warnBox : styles.infoBox}>
          ...
          {needed > available && available > 0 && (
            <span style={{ color: '#b45309', marginLeft: 8 }}>
              ⚠ {coderList.length} coders × {form.charts_per_coder} charts = {needed} needed.
              Some charts will repeat across coders.
            </span>
          )}
          {available === 0 && <span style={{ color: '#dc2626', marginLeft: 8 }}>⚠ No keyed charts match — upload answer keys first.</span>}
        </div>
      )}

      <div style={styles.formGroup}>
        <label style={styles.label}>Coders <span style={styles.hint}>(one name per line)</span></label>
        <textarea style={styles.textarea} rows={6}
          placeholder={'Sarah M\nJohn K\nPriya R'}
          value={form.coders}
          onChange={e => setForm(f => ({ ...f, coders: e.target.value }))} />
        {coderList.length > 0 && <span style={styles.hint}>{coderList.length} coder(s)</span>}
      </div>
