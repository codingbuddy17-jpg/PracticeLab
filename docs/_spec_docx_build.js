  ...
  sections: [{
    ...
    children: separate([
      ...
      rich([{ t: 'E/M answer keys → ' }, { t: 'em_answer_keys', code: true },
             { t: ', one row per chart across 47 columns: medical decision-making element counts, derived complexity levels, E/M code and modifier, encounter category, critical care minutes, and diagnosis and procedure codes as JSON.' }]),
      ...
      h3('JSON columns'),
      rich([{ t: '19 columns across the schema are typed ' }, { t: 'JSON', code: true },
      ...
      code([
        ...
        '    _run_migrations()                       # 21 CREATE TABLE + ~100 ALTER statements',
      ]),
      ...
      rich([{ t: 'Three tables were originally defined with ' }, { t: 'INTEGER PRIMARY KEY', code: true },
             { t: ' and had sequences attached by later migration — ' },
             { t: 'practice_sessions, practice_chart_drafts, practice_results', code: true },
             { t: '. These warrant particular attention.' }]),
      h2('5.3 Service checks'),
      code(['curl -s https://<internal-api-host>/health', '# {"status":"ok"}']),
      rich([{ t: '/health', code: true }, { t: ' confirms process availability and does not access the database. For a database read:' }]),
