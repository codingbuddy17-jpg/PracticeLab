...
GENERATE_PAYLOAD = {
    ...
    "duration_minutes": 60,
    "total_questions": 6,
    "specialty_mix": [{"specialty": "IPDRG", "pct": 100, "topic_filter": ""}],
...
def generate_and_get_token(client, db) -> str:
    ...
    r = client.post("/api/assessment/generate", json=GENERATE_PAYLOAD)
...
class TestSessionLoad:
    def test_get_session_info(self, client, db):
        ...
        r = client.get(f"/api/assessment/take/{token}")
        assert r.status_code == 200

    def test_session_info_has_questions(self, client, db):
        token = generate_and_get_token(client, db)
        data = client.get(f"/api/assessment/take/{token}").json()
        assert "questions" in data
        assert len(data["questions"]) == 6

    def test_invalid_token_returns_404(self, client, db):
        r = client.get("/api/assessment/take/ASM-INVALID1")
...
class TestSessionStart:
    def test_start_session(self, client, db):
        token = generate_and_get_token(client, db)
        r = client.post(f"/api/assessment/take/{token}/start")
        assert r.status_code == 200

    def test_start_sets_status_active(self, client, db):
        token = generate_and_get_token(client, db)
        client.post(f"/api/assessment/take/{token}/start")
        data = client.get(f"/api/assessment/take/{token}").json()
...
class TestAnswerSaving:
    def _started_session(self, client, db):
        token = generate_and_get_token(client, db)
        client.post(f"/api/assessment/take/{token}/start")
        questions = client.get(f"/api/assessment/take/{token}").json()["questions"]
        return token, questions

    def test_save_answer(self, client, db):
        token, qs = self._started_session(client, db)
        r = client.post(f"/api/assessment/take/{token}/answer", json={
            "question_index": 0,
...
class TestSessionSubmit:
    def _submit_session(self, client, db, answer_all=True):
        ...
        if answer_all:
            for i, q in enumerate(questions):
                client.post(f"/api/assessment/take/{token}/answer", json={
                    "question_index": i,
                    "question_id": q.get("question_id", str(q.get("id", ""))),
                    "selected_answer": "A",
                })
    ...
    def test_submit_returns_200(self, client, db):
        ...
        r = client.post(f"/api/assessment/take/{token}/submit", json={"auto_submitted": False})
        assert r.status_code == 200

    def test_submit_returns_score(self, client, db):
        token, _ = self._submit_session(client, db)
        data = client.post(f"/api/assessment/take/{token}/submit",
    ...
    def test_submit_twice_returns_error(self, client, db):
        token, _ = self._submit_session(client, db)
        client.post(f"/api/assessment/take/{token}/submit", json={"auto_submitted": False})
        r = client.post(f"/api/assessment/take/{token}/submit", json={"auto_submitted": False})
...
class TestSessionsList:
    def test_list_sessions_for_assessment(self, client, db):
        seed_question_pool(db)
        gen = client.post("/api/assessment/generate", json=GENERATE_PAYLOAD).json()
        assessment_id = gen["assessment_id"]
        r = client.get(f"/api/assessment/{assessment_id}/sessions")
        assert r.status_code == 200
        data = r.json()
    ...
    def test_session_row_has_token(self, client, db):
        ...
        gen = client.post("/api/assessment/generate", json=GENERATE_PAYLOAD).json()
        assessment_id = gen["assessment_id"]
        sessions = client.get(f"/api/assessment/{assessment_id}/sessions").json()["sessions"]
